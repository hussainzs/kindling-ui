...
### What was built
- Added [client/components/MilestoneCard.tsx](client/components/MilestoneCard.tsx) as a dedicated sticky-note card component. It owns note display styling with small tilt, centered pin, handwriting text, and palette-based tone variants.
- Added [client/components/MilestoneDraftInput.tsx](client/components/MilestoneDraftInput.tsx) as the editable draft milestone input area. Includes `add milestone` and `ignore` actions.
